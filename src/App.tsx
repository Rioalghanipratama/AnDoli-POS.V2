...
export default function App() {
  ...
  const [confirmPrintData, setConfirmPrintData] = useState<Transaction | null>(null);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      // Automatically clear printData after triggering the browser printer window
      setPrintData(null);
    }, 150);
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === 'Enter') {
        if (buffer.length > 0) {
          const product = products.find(p => p.id === buffer || p.name.toLowerCase() === buffer.toLowerCase());
          if (product) {
            addToCart(product);
          }
          buffer = '';
  ...
  const generateReceiptText = (t: Transaction | null): string => {
    if (!t) return '';
    const divider = '================================\n';
    const subDivider = '--------------------------------\n';
    let text = '';
    text += divider;
    text += '            AnDoli              \n';
    text += '     Anda tumbas saya Doli      \n';
    text += divider;
    text += `NO  : ${t.id}\n`;
    text += `TGL : ${new Date(t.date).toLocaleString('id-ID')}\n`;
    text += `S/N : ${t.customerName || 'Tamu Anonim'}\n`;
    text += subDivider;
    t.items.forEach(item => {
      const nameCol = item.name.substring(0, 16).padEnd(16, ' ');
      const qtyCol = `x${item.quantity}`.padEnd(5, ' ');
      const priceCol = formatCurrency(item.price * item.quantity).padStart(11, ' ');
      text += `${nameCol}${qtyCol}${priceCol}\n`;
    });
    text += subDivider;
    text += `TOTAL               ${formatCurrency(t.total).padStart(12, ' ')}\n`;
    if (t.pointsEarned > 0) {
      text += `POIN DIDAPAT        +${t.pointsEarned} poin\n`;
    }
    text += subDivider;
    text += '         Terima Kasih!          \n';
  ...
  const handlePrintReceipt = (transaction: Transaction | null) => {
    if (!transaction) return;
    
    // Attempt standard screen print first.
    // In React, calling window.print() of the main window is the most robust and standard approach.
    // Because modern browsers block nested iframe printing under sandbox regulations, standard window.print()
    // works beautifully once the application is opened in its own window or preview pane.
    try {
      window.print();
    } catch (e) {
      console.error("Window print failed, attempting iframe print", e);
      
      // Fallback: Custom Iframe Injection as secondary mechanism
        ...
        <html>
          <head>
            ...
            <style>
              ...
              .border-b { border-bottom: 2px dashed black; padding-bottom: 10px; margin-bottom: 10px; }
          ...
          <body>
            <div class="receipt-container">
              <div class="text-center mb-6">
                <h2 class="font-black" style="font-size:20px; margin:0 0 4px 0;">AnDoli</h2>
                <p style="font-size:9px; letter-spacing:1px; margin:0;">Anda tumbas saya Doli</p>
              </div>
              <div class="border-b"></div>
              <div class="mb-4 font-bold">
                <p style="margin:2px 0;">NO  : ${transaction.id}</p>
                <p style="margin:2px 0;">TGL : ${new Date(transaction.date).toLocaleString('id-ID')}</p>
                <p style="margin:2px 0;">S/N : ${transaction.customerName || 'Tamu Anonim'}</p>
              </div>
              <div class="border-b"></div>
              <table class="mb-4 font-bold">
                <tbody>
                  ${transaction.items.map(item => `
                    <tr>
                      <td style="padding-right: 8px;">${item.name}</td>
              ...
              <div class="border-b"></div>
              <div style="display: flex; justify-content: space-between;" class="font-black mb-2">
                <span style="font-size:14px;">TOTAL</span>
                <span style="font-size:14px;">${formatCurrency(transaction.total)}</span>
              </div>
              ...
              ` : ''}
              <div class="text-center mt-8" style="font-size: 9px; opacity: 0.6;">
      ...
      if (iframeDoc) {
        ...
        setTimeout(() => {
          try {
            ...
            console.error("Iframe print blocked as well:", iframeErr);
          }
          setTimeout(() => {
  ...
  const handleCheckout = () => {
    ...
    setConfirmPrintData(newTransaction);
  };
  ...
  return (
    <>
    <div className="print:hidden flex flex-col lg:flex-row h-screen bg-forma-bg text-forma-ink font-sans overflow-hidden">
      ...
      <main className="flex-1 flex flex-col min-w-0 border-forma-ink pb-16 lg:pb-0 lg:border-r-2 overflow-hidden">
        ...
        <header className="p-4 lg:p-8 flex justify-between items-center lg:items-end border-b-2 border-forma-ink bg-forma-bg sticky top-0 z-30">
          <div>
            <h1 className="text-xl lg:text-3xl font-black tracking-tighter leading-none">AnDoli</h1>
            <p className="hidden lg:block text-[10px] tracking-widest font-black opacity-30 mt-1">Anda tumbas saya Doli</p>
          </div>
          
          <div className="flex items-center gap-4 lg:gap-8">
            <div className="flex flex-col items-end">
              <span className="text-[8px] lg:text-[10px] uppercase tracking-widest font-black opacity-30">Pendapatan</span>
            ...
            {view === 'dashboard' && (
              ...
              >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  {[
                    { label: 'Pendapatan', value: formatCurrency(totalRevenue), icon: TrendingUp },
                    { label: 'Pesanan', value: transactions.length, icon: History },
                    { label: 'Pelanggan', value: customers.length, icon: Users },
                    { label: 'Rata-rata TRx', value: formatCurrency(transactions.length ? totalRevenue / transactions.length : 0), icon: Star },
                  ].map((stat, i) => (
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  ...
                  <div className="bg-white p-6 border-2 border-forma-ink">
                    <h4 className="text-sm font-black uppercase tracking-widest mb-6">Peringkat Loyalitas</h4>
                    <div className="space-y-4">
                      {customers.sort((a,b) => b.points - a.points).slice(0, 5).map((c, i) => (
                        <div key={i} className="flex items-center justify-between py-3 border-b border-forma-ink last:border-0 border-dashed">
                          ...
                          <span className="text-sm font-black text-forma-accent">{c.points} poin</span>
                        </div>
                      ))}
                    </div>
                  </div>
                    ...
                    <input 
                      ...
                      onKeyDown={(e) => {
                        if (e.key === 'Enter' && searchTerm) {
                          const exactMatch = products.find(p => p.id === searchTerm || p.name.toLowerCase() === searchTerm.toLowerCase());
                          if (exactMatch) {
                    ...
                    >
                      <div>
                        <div className="flex justify-between items-start mb-2">
                          ...
                          <span className="text-[10px] font-black">{product.stock} tersisa</span>
                        </div>
            ...
            {view === 'produk' && (
              <div className="bg-white border-2 border-forma-ink overflow-hidden">
                <div className="p-6 border-b-2 border-forma-ink flex justify-between items-center bg-forma-bg/50">
                   <h4 className="text-sm font-black uppercase tracking-widest">Manajemen Inventaris</h4>
                   <button 
                  ...
                  <table className="w-full text-left">
                    <thead>
                      <tr className="border-b-2 border-forma-ink text-[10px] font-black uppercase tracking-widest">
                        <th className="p-6">ID Katalog</th>
                    ...
                    <tbody className="divide-y border-forma-ink">
                      {products.map(p => (
                        <tr key={p.id} className="hover:bg-forma-bg transition-colors text-sm">
                          ...
                          <td className="p-6 space-x-4 flex-nowrap min-w-max">
                             <button 
                               onClick={() => {
                                 setEditingItem(p);
                                 setModalType('product');
                               }}
                               className="text-[10px] font-black uppercase text-blue-600 hover:opacity-60 mr-2"
                             >
                               Ubah
                             </button>
                             <button 
            ...
            {view === 'pelanggan' && (
              <div className="space-y-6">
                <div className="flex justify-between items-end">
                   <h4 className="text-xl font-black uppercase tracking-tighter">Direktori Pelanggan</h4>
                   <button 
                    ...
                    <div key={c.id} className="bg-white border-2 border-forma-ink p-8 relative group hover:shadow-[8px_8px_0px_0px_#1A1A1A] transition-all">
                      ...
                      <div className="grid grid-cols-2 gap-8 border-t-2 border-forma-ink pt-6">
                        <div className="space-y-1">
                          <span className="text-[9px] font-black uppercase tracking-widest opacity-40">Poin Loyalitas</span>
                          <p className="text-2xl font-black text-forma-accent">{c.points}</p>
                        </div>
                        <div className="space-y-1 text-right">
                          <span className="text-[9px] font-black uppercase tracking-widest opacity-40">Total Belanja</span>
                      ...
                      <div className="mt-8 flex justify-between items-center bg-forma-bg/50 p-3 text-[10px] font-black uppercase tracking-widest border-t border-forma-ink border-dashed">
                         <span className="opacity-40">Kunjungan #{c.transactionsCount}</span>
                         <span className="opacity-40">Terakhir: {c.lastVisit}</span>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            {view === 'riwayat' && (
               <div className="bg-white border-2 border-forma-ink overflow-hidden">
                 <div className="p-6 border-b-2 border-forma-ink bg-forma-bg/50">
                    <h4 className="text-sm font-black uppercase tracking-widest">Arsip Transaksi</h4>
                 </div>
                 <div className="overflow-x-auto">
                   <table className="w-full text-left">
                     <thead>
                       <tr className="border-b-2 border-forma-ink text-[10px] font-black uppercase tracking-widest">
                         <th className="p-6">No. Ref</th>
                         <th className="p-6">Akun</th>
                         <th className="p-6">Nilai</th>
                         <th className="p-6">Waktu</th>
                         <th className="p-6 text-right">Verifikasi</th>
                       </tr>
                     </thead>
                     <tbody className="divide-y border-forma-ink">
                       {transactions.map(t => (
                         <tr key={t.id} className="hover:bg-forma-bg text-sm">
                           ...
                           <td className="p-6 font-black uppercase">{t.customerName || 'Tamu Anonim'}</td>
                           ...
                           <td className="p-6 text-right flex justify-end items-center gap-2">
                              <span className="px-2 py-1 text-[8px] font-black uppercase tracking-widest border border-forma-ink bg-forma-ink text-forma-bg">Divalidasi</span>
                              <button 
                                onClick={() => setConfirmPrintData(t)}
                                className="px-3 py-1 text-[8px] font-black uppercase tracking-widest border border-forma-ink hover:bg-forma-accent hover:text-white transition-colors"
                              >
                                Cetak
                              </button>
                           </td>
      ...
      <aside className={cn(
        ...
        <div className="lg:hidden p-4 border-b-2 border-forma-ink flex justify-between items-center bg-forma-bg shrink-0 sticky top-0 z-20">
           <h4 className="font-black uppercase tracking-widest">Daftar Pesanan</h4>
           <button onClick={() => setIsSidebarOpen(false)} className="font-black px-4 py-2 border-2 border-forma-ink">TUTUP</button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar flex flex-col">
          <div className="p-6 lg:p-8 border-b-2 border-forma-ink flex flex-col items-center bg-forma-muted">
            <span className="text-[10px] font-black uppercase tracking-[0.2em] mb-6 opacity-40">Profil Pelanggan</span>
            ...
            {selectedCustomer ? (
              <div className="bg-forma-bg border-2 border-forma-ink p-6 w-full shadow-[4px_4px_0px_0px_#1A1A1A]">
                ...
                <div className="grid grid-cols-2 gap-4 border-t border-forma-ink pt-4">
                  <div>
                    <span className="block text-[8px] font-black uppercase opacity-40">Total Belanja</span>
                    <span className="block text-sm font-black">{formatCurrency(selectedCustomer.totalSpent)}</span>
                  </div>
                  <div>
                    <span className="block text-[8px] font-black uppercase opacity-40">Saldo Poin</span>
                    <span className="block text-sm font-black">{selectedCustomer.points} poin</span>
                  </div>
                </div>
                <button 
          ...
          <div className="flex-1 p-8">
            <span className="text-[10px] font-black uppercase tracking-[0.2em] mb-6 block opacity-40">Daftar Pesanan</span>
            
            {cart.length === 0 ? (
              <div className="py-20 flex flex-col items-center justify-center opacity-10">
                <ShoppingCart size={48} />
                <p className="font-black uppercase tracking-widest text-xs mt-4">Kosong</p>
              </div>
            ) : (
              <div className="space-y-4">
                {cart.map(item => (
          ...
          <div className="p-6 lg:p-8 border-t-2 border-forma-ink bg-forma-muted mt-auto">
            <div className="space-y-1 mb-6 lg:mb-8">
              ...
              <div className="flex justify-between text-[10px] font-black uppercase text-forma-accent">
                <span>Poin Didapat</span>
                <span>+{potentialPoints} poin</span>
              </div>
        ...
        {isCheckoutOpen && (
          ...
          >
            ...
            >
              ...
              <h3 className="text-3xl font-black uppercase tracking-tighter mb-4">Selesaikan Beli?</h3>
              <p className="text-sm font-bold opacity-60 mb-12">Konfirmasi penerimaan sebesar {formatCurrency(cartTotal)} untuk pesanan ini.</p>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-forma-ink/80 backdrop-blur-sm">
            ...
            >
              <h3 className="text-2xl font-black uppercase tracking-tighter mb-8">
                {editingItem ? 'Ubah' : 'Tambah'} {modalType === 'product' ? 'Produk' : 'Pelanggan'}
              </h3>
              ...
              >
                {modalType === 'product' ? (
                  <>
                    <Input label="Nama Produk" name="name" defaultValue={editingItem?.name} required />
                    <Input label="Kategori" name="category" defaultValue={editingItem?.category} required />
                    <div className="grid grid-cols-2 gap-4">
                      <Input label="Harga" name="price" type="number" defaultValue={editingItem?.price} required />
                      <Input label="Stok" name="stock" type="number" defaultValue={editingItem?.stock} required />
                    </div>
                  </>
                ) : (
                  <>
                    ...
                    <Input label="Nomor Telepon" name="phone" required />
                  </>
                )}
      ...
      {/* Pop-up Interaktif: Cetak Struk? Ya / Tidak */}
      <AnimatePresence>
        {confirmPrintData && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-forma-ink/80 backdrop-blur-sm print:hidden">
            <motion.div 
              initial={{ scale: 0.95, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.95, opacity: 0 }}
              className="bg-forma-bg border-4 border-forma-ink p-8 max-w-sm w-full shadow-[8px_8px_0px_0px_#1A1A1A] text-center"
            >
              <div className="w-16 h-16 border-4 border-forma-ink flex items-center justify-center mx-auto mb-6 bg-forma-accent text-white rounded-none">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={3} stroke="currentColor" className="w-8 h-8">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M6.72 13.829c-.24.03-.48.062-.72.096m.72-.096a42.415 42.415 0 0 1 10.56 0m-10.56 0L6.34 18m10.94-4.171c.24.03.48.062.72.096m-.72-.096L17.66 18m0 0 .229 2.523a1.125 1.125 0 0 1-1.12 1.227H7.231c-.615 0-1.11-.474-1.12-1.088L6 18m12 0H6m12 0a2.25 2.25 0 0 0 2.25-2.25V11.25a2.25 2.25 0 0 0-2.25-2.25H4.5A2.25 2.25 0 0 0 2.25 11.25v4.5A2.25 2.25 0 0 0 4.5 18M15 9V4.5A2.25 2.25 0 0 0 12.75 2.25h-1.5A2.25 2.25 0 0 0 9 4.5V9m3 11.25a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0Z" />
                </svg>
              </div>
              <h3 className="text-lg font-black uppercase tracking-tight text-forma-ink">Cetak Struk?</h3>
              <p className="text-xs font-bold opacity-60 mt-2 mb-6 leading-relaxed">
                Apakah Anda ingin mencetak struk untuk transaksi #{confirmPrintData.id}?
              </p>
              <div className="flex gap-4">
                <button 
                  onClick={() => setConfirmPrintData(null)}
                  className="flex-1 border-2 border-forma-ink py-4 text-[10px] font-black uppercase tracking-widest hover:bg-forma-ink hover:text-forma-bg transition-colors"
                >
                  Tidak
                </button>
                <button 
                  onClick={() => {
                    setPrintData(confirmPrintData);
                    setConfirmPrintData(null);
                  }}
                  className="flex-1 bg-forma-ink text-forma-bg py-4 text-[10px] font-black uppercase tracking-widest hover:bg-forma-accent border-2 border-forma-ink transition-colors"
                >
                  Ya, Cetak
    ...
    {/* Thermal Receipt Print Layout - Invisible on screen, formats on printer */}
    {printData && (
      <div className="hidden print:block w-[80mm] text-black font-mono text-sm leading-tight bg-white p-4 mx-auto">
        <div className="text-center mb-6">
          <h2 className="text-2xl font-black tracking-tighter">AnDoli</h2>
          <p className="text-[10px] tracking-widest">Anda tumbas saya Doli</p>
        </div>
        <div className="border-b-2 border-dashed border-black mb-4"></div>
        <div className="mb-4 text-xs font-bold leading-relaxed">
          <p>NO  : {printData.id}</p>
          <p>TGL : {new Date(printData.date).toLocaleString('id-ID')}</p>
          <p>S/N : {printData.customerName || 'Tamu Anonim'}</p>
        </div>
        <div className="border-b-2 border-dashed border-black mb-4"></div>
        <table className="w-full mb-4 text-xs font-bold">
          <tbody>
            {printData.items.map(item => (
              <tr key={item.id} className="align-top pb-2">
                <td className="pr-2 pb-2">{item.name}</td>
                <td className="text-center pb-2">x{item.quantity}</td>
                <td className="text-right pb-2">{formatCurrency(item.price * item.quantity)}</td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        {printData.pointsEarned > 0 && (
          <div className="flex justify-between text-[10px] uppercase tracking-widest mb-4">
            <span>Poin Didapat</span>
            <span>+{printData.pointsEarned} poin</span>
          </div>
        )}
        <div className="text-center mt-8 text-[10px] uppercase tracking-widest opacity-60">
          <p>Terima Kasih!</p>
